...
class DigitalTwin:
    ...
    def create_dtObject(self, obj_id):
        n_objs = sum([x.startswith(obj_id) for x in self.dt_objectcs_list.keys()])
        unique_obj_name = f"{obj_id}__{n_objs:02}"
        meshpath = os.path.join("data", "models", obj_id, f"{obj_id}.ply")

        new_obj = DTObject(name=unique_obj_name, meshpath=meshpath, show_axes=True)
    ...
    def camera_thread(self):
        ...
        objectDetector.weights_path = os.path.join(obj_data, "yolo.weights")
        ...
        while not self.stop_flag:
            ...
            if not detected or self.trigger_darknet:
                bbox = None
                ...
                if len(detections) > 0:
                    index = np.random.randint(len(detections)-1) if self.trigger_darknet else -1
                    bbox_xywh = detections[index][2]
                    confidence = detections[index][1]
                    bbox = objectDetector.yolo_bbox_2_original(bbox_xywh, frame.rgb.shape[:2])
                    self.trigger_darknet = False
            else:
                ...
                affine_matrix = invert_homogeneous(frame.extrinsic) @ dt_obj.pose
            ...
            if not self.freeze_object:
                ...
                if bbox is not None and poseDetection is not None:
                    try:
                        detected, affine_matrix = poseDetection.inference(
                            bbox,
                            frame.rgb.copy(),
                            frame.depth,
                            frame.intrinsic,
                            use_icp=self.use_icp,
                        )  # affine_matrix.shape = 3x4 !
                ...
                if detected:
                    ...
                    if prev_pose is not None:
                        v = np.linalg.norm(current_pose[:3, 3] - prev_pose[:3, 3]) / dt  # m / sec
                        omega = R.from_matrix(current_pose[:3, :3]) * R.from_matrix(
                            prev_pose[:3, :3].T
                        )
                        ...
                        if vel_limit or omega_limit:
                            ...
                            print(f"[{time_stamp()}] Using darknet because of {reason}")
            ...
            self.obstruction_pcd.update_from_scene(frame, dt_obj, affine_matrix)
            ...
            if self.draw_object:
                ...
                if dt_obj.active:
                    annotated = poseDetection.draw_obj(annotated, affine_matrix, frame.intrinsic)
    ...
    def control_robot(self, control):
        self.gripper.set_target_force(self.gripper.target_force + control[6])
        self.gripper.set_target_width(self.gripper.target_width + control[7])

        if np.any(control[:6]):
            new_pose = np.eye(4)
            new_pose[:3, :3] = R.from_euler("XYZ", control[3:6]).as_matrix()
            new_pose[:3, 3] = control[0:3]

            self.gripper.move_to_pose(new_pose, incremental=True, local_frame=True)
