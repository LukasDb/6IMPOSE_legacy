...
PYTHON="/root/micromamba/envs/tf/bin/python"
...
if [ $? -eq 0 ]; then
    echo "3d_interpolation/tf_interpolate_so.so compiled successfully."
else
    echo "3d_interpolation/tf_interpolate_so.so compilation failed."
    exit 1
fi
...
if [ $? -eq 0 ]; then
    echo "grouping/tf_grouping_so.so compiled successfully."
else
    echo "grouping/tf_grouping_so.so compilation failed."
    exit 1
fi
...
if [ $? -eq 0 ]; then
    echo "sampling/tf_sampling_so.so compiled successfully."
else
    echo "sampling/tf_sampling_so.so compilation failed."
    exit 1
fi
